...
class Session:
    ...
    def send_request(self):
        ...
        response = requests.post("http://localhost:8000/functions/trigger",
                                    ...
                                    data=json.dumps({"http_body": json.dumps({"corr_id": str(corr_id), "body": json.loads(data)},
                                                                                default=lambda x: x.decode("utf-8")),
                                                    "http_headers": self.protocol._headers,
                                                    "http_path": http_path,
                                                    "http_method": "ANY"}))
        response_bodies = response.json().get("all_logs") # .get("instance_ids")
