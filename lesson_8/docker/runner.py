...
tracer = tracing.init_tracer(f"http|{verb}|{path}")

with tracing.start_span(tracer, "run-function"):
    with tracing.start_span(tracer, "configure-pre-user-code") as span:
        tracing.set_attr(span, "headers", headers)
        tracing.set_attr(span, "body", body)
    ...
    with tracing.start_span(tracer, "invoke-user-code") as span:
        context = dict(message_queue_uri=message_queue_uri, tracing_context=span, tracer=tracer)

        tracing.set_attr(span, "context", context)
        response = handler.handler(message, context)

        tracing.set_attr(span, "response", response)

    with tracing.start_span(tracer, "send-response") as span:
